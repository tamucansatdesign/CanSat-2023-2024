...
namespace SensorFusion
{
    class KalmanFilter 
    {
        public:
            ...
            void update(float gps_longitude, float gps_latitude, float bmp_altitude, float gps_altitude, float gps_speed, float gps_angle, float airspeed, float tilt_x, float tilt_y);

            BLA::Matrix<state_dim, 1> get_state();
        ...
        private:
            ...
            const float sigma_gps_position = 1.8;                   // Ultimate GPS Breakout position uncertainty in m (from datasheet)
            const float sigma_gps_velocity = 0.1;                   // Ultimate GPS Breakout velocity uncertainty in m/s (from datasheet)
            // const float sigma_airspeed = 
            // const float sigma_tilt = 
